...
extern void keyboard_handler_window();
...
int SizeOfArray = *(&fn_table + 1) - fn_table;
int SizeOfFileArray = *(&file_table + 1) - file_table;
...
void init_idt_window() {
	// Get the address of the keyboard_handler code in kernel.asm as a number
	unsigned int offset = (unsigned int)keyboard_handler_window;
...
void handle_keyboard_interrupt_window() {
	// Write end of interrupt (EOI)
	...
	if (status & 0x1) {
		...
		if (keycode == 28) {
			...
			printk(" TA-DAA, interactive windows!");
			newline();
			printk(" Please wait...");
			sleep(0x02FFFFFFF);
			textMode();
			...
			update("        This is an update!        ", 0, 7);
		} else if ( keycode == 4 ) {
			update("   Press 1 to interact with me!   ", 0, 7);
		}
	}
}
...
void handle_keyboard_interrupt2() {
	...
	if (status & 0x1) {
		...
		if (keycode == 28) {
			...
			// Get the length of the module name and store it as an int.
			//int LengthOfModuleName = stringLength(fn_table[loop].name);
			//int LengthOfModuleName2 = stringLength(fn_table[loop++].name);

			checkModule(); // Check if user typed a module name

			int lenModOne = stringLength(ModuleNames[0]);
			int lenModTwo = stringLength(ModuleNames[1]);
			int lenModThree = stringLength(ModuleNames[2]);
			int lenModFour = stringLength(ModuleNames[3]);
			...
			if (streq(command_buffer, command_len, "ls", 2)) {
				...
				newline();
			} else if (streq(command_buffer, command_len, ModuleNames[0], lenModOne)) {
				clear_screen2(0);
				textMode();
				call_function(ModuleNames[0])();
			} else if (streq(command_buffer, command_len, ModuleNames[1], lenModTwo)) {
				clear_screen2(0);
				textMode();
				call_function(ModuleNames[1])();
			} else if (streq(command_buffer, command_len, ModuleNames[2], lenModThree)) {
				clear_screen2(0);
				textMode();
				call_function(ModuleNames[2])();
			} else if (streq(command_buffer, command_len, ModuleNames[3], lenModFour)) {
				clear_screen2(0);
				textMode();
				call_function(ModuleNames[3])();
				...
				for ( int i=0; i<4; i++ ) {
					if (file_table[i].name != FileName) {
						if (file_table[i].name != "empty") {
							file_table[i].name = "empty";
							file_table[i].contents = "empty";
						}
					}	
				}
				...
				// If files are already deleted or does not exist, 
				// return an error. If not, then delete all files.
				for ( int i=0; i<4; i++ ) {
					if (file_table[i].name != "empty") {
						FileIndex = 0;
						newline();
						//newline();
						print_to_screen("File ", 2, 0);
						printk("'");
						printk(file_table[i].name);
						printk("'");
						print_to_screen(" deleted.", 2, 0);
				...
				newline();
				print_to_screen("Press Q to hide Quick Menu", 2, 0);
				newline();
...
void handle_keyboard_interrupt_christmas() {
	...
	if (status & 0x1) {
		...
		if (keycode == 28) {
			...
			DrawStartMenu2();
		}else if (keycode == 16) { // 'q'
			isMenuOpen2 = 1; // Menu is closed
			
			if ( isWindowOpen2 == 0 ) { // Window is open
				desktopChristmas();
			} else if ( isWindowOpen2 == 1 ) { // Window is closed
				closeWindowChristmas();
			}
		}else if (keycode == 14) {
			// BACKSPACE: Move back one unless on prompt
...
void handle_keyboard_interrupt() {
	...
	if (status & 0x1) {
		...
		if (keycode == 28) {
			...
			isMenuOpen = 1; // Menu is closed
			
			if ( isWindowOpen == 0 ) { // Window is open
				desktop();
			} else if ( isWindowOpen == 1 ) { // Window is closed
				closeWindow();
			}
		}else if (keycode == 14) {
			// BACKSPACE: Move back one unless on prompt
