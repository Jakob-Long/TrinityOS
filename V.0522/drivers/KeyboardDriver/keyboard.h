...
void handle_keyboard_interrupt2() {
	...
	if (status & 0x1) {
		...
		if (keycode == 28) {
			...
			} else if (streq(command_buffer, command_len, fn_table[0].name, 4)) {
				textMode();
				call_function("taya")();
			} else if (streq(command_buffer, command_len, fn_table[2].name, 4)) {
				textMode();
				call_function("demo")();
			} else if (streq(command_buffer, command_len, fn_table[3].name, 6)) {
				textMode();
				call_function("create")();
			} else if (streq(command_buffer, command_len, fn_table[4].name, 4)) {
				textMode();
				call_function("read")();
			} else if (streq(command_buffer, command_len, fn_table[5].name, 4)) {
				textMode();
				call_function("boot")();
				textMode();
