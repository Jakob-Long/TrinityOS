...
void handle_keyboard_interrupt_login() {
	...
	if (status & 0x1) {
		...
		if (keycode == 28) {
			// ENTER : Newline
			// check if password right, if so login
			if (streq(command_buffer, command_len, "admin", 5)) main("desktop1");
			else {
				//lockScreen();
				command_buffer[0] = 0;
...
void handle_keyboard_interrupt2() {
	...
	if (status & 0x1) {
		...
		if (keycode == 28) {
			...
			if (streq(command_buffer, command_len, "ls", 2)) {
			...
			} else if (streq(command_buffer, command_len, array[0], 3)) {
				module_init();
				sleep(0x10FFFFFF);
				textMode();
			} else if (streq(command_buffer, command_len, array[1], 4)) {
				//module_init();
				clear_screen2(0);
				textMode();
				newline();
				...
				//functions[0];
			} else if (streq(command_buffer, command_len, array[2], 4)) {
				//module_init();
				clear_screen2(0);
				textMode();
				newline();
